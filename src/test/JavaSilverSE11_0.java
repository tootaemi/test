...
//		
//		このコードが10を出力する理由は、
//		Javaがプリミティブ型の引数をメソッドに渡す際に「値渡し」を使用するためです。
//		つまり、modifyメソッドにnumの値を渡すとき、
//		その値のコピーが作成されてメソッドに渡されます。
//		メソッド内でこのコピーに対して変更が加えられても、
//		元の変数（この場合はSampleクラスのインスタンス変数num）の値には影響しません。
//
//		プログラムの動作は以下のようになります：
//
//		Sampleクラスのインスタンスsが、コンストラクタに10を引数として渡して作成されます。
//		これにより、s.numの値は10に初期化されます。
//		modify(s.num);が呼び出されます。ここで、s.numの値10が
//		modifyメソッドの引数numに「値渡し」されます。
//		modifyメソッド内でこの引数の値を2倍にしても、
//		これは引数のコピーに対する操作なので、sのnumフィールドの値には影響しません。
//		System.out.println(s.num);が実行され、
//		s.numの値（変更されていない、元の値10）が出力されます。
...
//		⇨　20が表示される。
//
//		Sampleクラスのインスタンスnumフィールドは、modifyメソッドによって2倍にされます。
//		そのため、最終的な出力は20になります。解説します：
//		Sampleクラスには、整数を引数に取るコンストラクタがあります。
//		このコンストラクタは、引数で渡された値をインスタンス変数numに代入します。
//		Mainクラスのmainメソッドで、Sampleクラスの新しいインスタンスが
//		整数10を引数にして作成されます。
//		作成されたSampleインスタンスSは、modifyメソッドに渡されます。このメソッドは、
//		引数で受け取ったSampleインスタンスのnumフィールドの値を2倍にします。
//		したがって、s.numの値は10から20に変更されます。
//		最後に、System.out.println(s.num);により、
//		s.numの値20がコンソールに出力されます。
//		このコードは、オブジェクト指向プログラミングの基本的な概念であるオブジェクトの状態の変更、
//		メソッドによるオブジェクトの操作、および参照によるオブジェクトの渡し方を示しています。
//		
